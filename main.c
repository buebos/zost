...
int main(int argc, char** argv) {
    ...
    Reader source_reader = reader_init(source_file);
    Tokenizer tokenizer = tokenizer_init(&source_reader, &status);
    ...
    foreach (Token, token, tokenizer) {
        /**
         * Mutates the abstract syntax tree (AST) or returns an error if there
         * was one.
         */
        status = parser_handle(&parser, token);

        if (status.code != 0) {
            return status.code;
        }

        status = analyzer_handle(&analyzer, token);
     ...
     */
    Writter asm_writter = reader_init(fopen("asm_temp", "w+"));
    Reader tac_reader = reader_init(fopen("tac_temp", "r"));

    asm_from_tac(&asm_writter, &tac_reader);
    /** Maybe also apply optimizations to the assembly file generated */
    // asm_optimize(&asm_writter);
    writter_close(&asm_writter);
    reader_close(&tac_reader);

    Writter bin_writter = reader_init(fopen("a.out", "w+"));
    Reader asm_reader = reader_init(fopen("asm_temp", "r"));

    bin_from_asm(&bin_writter, &asm_reader);
    writter_close(&bin_writter);
    reader_close(&asm_reader);
